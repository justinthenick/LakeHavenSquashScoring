...
test('legacy played flag without saved result is CHECK, not a silent gap',()=>{const x=JSON.parse(JSON.stringify(d));x.matches=[];x.fixtures[0].played=true;const report=c.reportFromStandings_(c.buildCompetitionStandings_(x,{}));assert.equal(report.lines[0].rows[0].byRound[0],'CHECK');assert(report.warnings[0].includes('no saved result'));});
